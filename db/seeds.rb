...
10.times do
  Vendor.create(name:Faker::Company.name,address:Faker::Address.city,Faker::Address.state])
end
 low_vendor_id = Vendor.first.id
 high_vendor_id = Vendor.last.id

10.times do
  Customer.create(name:Faker::Company.name,address:[Faker::Address.city,Faker::Address.state])
end
